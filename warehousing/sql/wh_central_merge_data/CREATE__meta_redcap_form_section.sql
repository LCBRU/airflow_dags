...
CREATE TABLE meta__redcap_form_section (
    ...
    meta__form_id INT NOT NULL,
    name_index NVARCHAR(500) NOT NULL,
    name NVARCHAR(MAX) NOT NULL,
    INDEX idx__redcap_form_section__name (name_index),
    FOREIGN KEY (meta__form_id) REFERENCES meta__redcap_form(id),
    UNIQUE (meta__form_id, name_index)
);

EXEC sp_MSforeachdb
@command1="IF '?' LIKE 'datalake_redcap_%'
BEGIN
	INSERT INTO warehouse_central.dbo.meta__redcap_form_section (meta__form_id, name_index, name)
    SELECT
        rf.id,
