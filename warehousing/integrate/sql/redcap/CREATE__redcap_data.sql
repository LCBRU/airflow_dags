...
SELECT DISTINCT
    m.cfg_redcap_instance_id,
    m.meta__redcap_project_id,
    m.meta__redcap_arm_id,
    m.meta__redcap_event_id,
    m.meta__redcap_form_id,
    m.meta__redcap_form_section_id,
    m.meta__redcap_field_id,
    m.meta__redcap_data_type_id,
    m.redcap_participant_id,
    ISNULL(rd.[instance], 1) AS instance,
    rd.value
...
JOIN (
	SELECT
		...
		mrdt.id AS meta__redcap_data_type_id,
...
) m ON m.cfg_redcap_instance_id = ' + CONVERT(NVARCHAR(10), @cfg_redcap_instance_id) + '
	AND m.redcap_project_id = rd.project_id
	AND m.redcap_event_id = rd.event_id 
	AND m.field_name = rd.field_name
	AND m.record = rd.record
'
